...
// random function from w3Shcool
function random(min, max) {
  return Math.floor(Math.random() * (max - min + 1)) + min;
}


// establish array for time
let timeArray=[];
for (let i = 6; i < 20; i++) {

  timeArray.push(`${i}:00`);
}
timeArray.push('Daily Location Total');
let objects=[];
let randomNumbersArray=[];

let avgCookieArray=[];
// // construction object
function SalmonCookies(name,time,minNumber,maxNumber,avgCookiePerPerson)
{
  this.name=name;
  this.timeArray=time;
  ...
  this.avgCookieArray=[];
  this.avgCookiePerPerson=[6.3,1.2,3.7,2.3,4.6];



  // prototype for generatr Numbers
  SalmonCookies.prototype.generateNumbers=function()
  {
    for (let i = 0; i < 14; i++) {

      let getRandom =random(this.minNumber,this.maxNumber);
      this.randomNumbersArray.push(getRandom);
    }

  };
  SalmonCookies.prototype.avgCookieNumber=function()
  {
    for (let i = 0; i < 5; i++) {
      for(let j=0;j<14;j++){
        let result=this.randomNumbersArray[j]*this.avgCookiePerPerson[i];
        this.avgCookieArray.push(result);
      }
    }
...
let seattle = new SalmonCookies('Seattle',timeArray,23,65);
seattle.generateNumbers();
seattle.avgCookieNumber();

console.log(seattle.randomNumbersArray);
console.log(seattle.avgCookieArray);



let Tokyo = new SalmonCookies('Tokyo',timeArray, 3 , 24,1.2);
let Dubai = new SalmonCookies ('Dubia',timeArray, 11, 38 ,3.7);
Dubai.generateNumbers();
// console.log(Dubai);

let Paris = new SalmonCookies('Paris',timeArray, 20 , 38,2.3 );
let Lima = new SalmonCookies('Lima',timeArray, 2 , 16 ,4.6);
Dubai.generateNumbers();
// console.log(Dubai);
...
for (let i = 0; i <timeArray.length; i++) {
  ...
  headingRow.textContent=timeArray[i];
...
for (let i = 0; i < 5; i++) {
  for (let j = 0; j < 14; j++) {
    let firstRow1=document.createElement('tr');
    let dataRows=document.createElement('td');
    firstRow1.appendChild(dataRows);
    dataRows.textContent=this.randomNumbersArray[j]*this.avgCookiePerPerson[i];
